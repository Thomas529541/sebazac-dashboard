...
<style>
    ...
    /* Style pour le bridge chart summary */
    .bridge-summary {
        font-size: 16px; font-weight: bold; text-align: center; 
        margin-bottom: 5px; padding: 8px; background-color: #31333F; border-radius: 8px; border: 1px solid #444;
    }
</style>
...
def load_data():
    try:
        # Assurez-vous que le fichier s'appelle bien ainsi dans votre GitHub
        ...
        if 'Période' in df_h.columns: df_h['Date'] = pd.to_datetime(df_h['Période'], dayfirst=True)
        
        cols_a = {'CA TTC': clean_curr, 'Quantité': pd.to_numeric}
        for col, func in cols_a.items():
            if col in df_a.columns: df_a[col] = func(df_a[col])
        if 'Période' in df_a.columns: df_a['Date'] = pd.to_datetime(df_a['Période'], dayfirst=True)
...
with st.sidebar:
    ...
    st.markdown("---")
    waterfall_comp = st.radio("Cascade Activités vs :", ["Mois Précédent (M-1)", "Année Précédente (N-1)"])

# --- PRÉPARATION DES PÉRIODES ---
mask_curr = (df_horaire['Date'].dt.year == annee_sel) & (df_horaire['Date'].dt.month == mois_sel)
df_curr = df_horaire[mask_curr]
...
mask_m1 = (df_horaire['Date'].dt.year == prev_y_m) & (df_horaire['Date'].dt.month == prev_m)
df_m1 = df_horaire[mask_m1]

# N-1
mask_n1 = (df_horaire['Date'].dt.year == annee_sel - 1) & (df_horaire['Date'].dt.month == mois_sel)
df_n1 = df_horaire[mask_n1]
...
# B. CASCADE CORRECTIVE
with c_right:
    # Définition des labels dynamiques
    ...
    if not df_act_curr.empty and not df_act_prev.empty:
        ...
        # Affichage du résumé en haut du graph
        st.markdown(f"""
        ...
        # Calcul des deltas par activité
        ...
        fig_water.update_layout(
            height=350, showlegend=False, yaxis=dict(range=[0, max_h], title="CA TTC (€)"),
...
# --- 5. SECTION HEATMAP ROBUSTE (ZÉROS & CONSOLIDATION) ---
st.subheader("🔥 Heatmap Hebdomadaire (Comparaison Fiabilisée)")
...
if not df_curr.empty:
    ...
    def get_hm_data(df_source, indicator):
        # 1. Consolidation (Somme Caisses)
        df_conso = df_source.groupby(['Date', 'Heure']).sum(numeric_only=True).reset_index()
    ...
    # Prép données graphiques
    z_values = matrix_curr.values
    text_values = np.round(matrix_curr.values).astype(str) # Par défaut, juste le chiffre
    colors = "Blues"
    zmin, zmax = None, None
    ...
    if "Évolution" in vue:
        ...
        if not df_ref.empty:
            ...
            # CALCUL INTELLIGENT DES VARIATIONS
            # On crée des matrices numpy pour manipuler facilement
            curr_vals = matrix_curr.values
            ref_vals = matrix_ref.values
            
            # Initialisation matrices résultats
            ...
            for r in range(rows):
                for c in range(cols):
                    ...
                    if v_ref == 0 and v_cur == 0:
                        ...
                        diff_pct[r, c] = 100 # Force bleu max
                        display_text[r, c] = "Ouv."
                    elif v_ref > 0 and v_cur == 0:
                        diff_pct[r, c] = -100 # Force rouge max
                        display_text[r, c] = "Ferm."
                    else:
                        pct = ((v_cur - v_ref) / v_ref) * 100
                        diff_pct[r, c] = pct
                        display_text[r, c] = f"{pct:+.0f}%"

            z_values = diff_pct
            text_values = display_text
            colors = "RdBu"
            zmin, zmax = -100, 100 # On bloque l'échelle
        else:
